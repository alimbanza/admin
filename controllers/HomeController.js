...
module.exports={
    ...
    actualisation : async(req,res)=>{  
        if(typeof req.session.org !== 'undefined' && typeof req.session.pseudo !== 'undefined' && typeof req.session.mdp !== 'undefined' && typeof req.session.id_compte !== 'undefined')  {
            try {
                let connexion1 = await utilisateurService.connexion_administrateur(req.session.pseudo)
                    utilisateur= await utilisateurService.check_pseudo_mdp_admin_plat(req.session.pseudo),
