...
module.exports={
    get_adherent_by_im_id_piece:async function(data){
        let adherent_=''
        const [adherent, metadata_] = await sequelize.query("SELECT users.*,(users.id+1) membre_partrnaire,etat_civils.libelle eta,users.id id_user,partenaires.*,partenaires.id id_org,type_employes.id id_type FROM users INNER JOIN employes ON employes.id_user=users.id INNER JOIN partenaires ON partenaires.id=employes.id_partenaire INNER JOIN etat_civils ON etat_civils.id=users.id_etat_civil INNER JOIN type_employes ON type_employes.id=employes.id_type_employe WHERE (users.numero_piece=:id_piece AND users.id_type_piece_ident=:id_tp_piece AND (CONCAT(LPAD(partenaires.id,4,0),'-',LPAD(users.id,6,0)))=:im)",
        {
            ...
            if(compte.membre_partrnaire == true){
                if(await this.check_type_compte_and_id_user(compte) =='' ){
                    ...
                    let compt = await this.search_compte_user(id_cmt),
                        ...
                        message=`Vous venez de créer un compte ${comp.dataValues.type_compte} : ${compt[0].compte} `,
