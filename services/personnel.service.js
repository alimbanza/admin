...
module.exports={
                            ...
                            await sequelize.transaction(async (t) => {
                               /* if(await this.check_pseudo_membre(data.prenom)==''){
                                ...
                                let numPhone= user.dataValues.telephone,
                                    ...
                                    à une structure
                                    Veuillez changer votre mot de passe pour
                                    le sécuriser.
                                    Merci.`,
    ...
    rep_personnel : async (dataMembre)=>{
        ...
        if(dataMembre.type == 1){
            if(dataMembre.id == 6){
                const [pers_, meta] = await sequelize.query("SELECT users.*,type_membres.type_membre libelle,type_membres.id id_type FROM users INNER JOIN membres ON membres.id_user=users.id INNER JOIN type_membres ON type_membres.id=membres.id_type_membre WHERE type_membres.id=:type_membre",
                {
                    replacements: { type_membre:dataMembre.id }
                })
                pers = JSON.parse(JSON.stringify(pers_))
            }else{
                const [pers_, meta] = await sequelize.query("SELECT users.*,agences.denomination,type_employes.libelle,type_employes.id id_type FROM users INNER JOIN employes ON employes.id_user=users.id INNER JOIN agences ON agences.id = employes.id_agence INNER JOIN type_employes ON type_employes.id=employes.id_type_employe WHERE type_employes.id=:type_membre",
                {
                    replacements: { type_membre:dataMembre.id }
                })
                pers = JSON.parse(JSON.stringify(pers_))
