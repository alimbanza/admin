...
module.exports={ 
    ...
    check_pseudo_mdp_admin_plat : async function(pseudo_){
        const [cmtt, metadata_] = await sequelize.query("SELECT agences.id id_o, employes.id_type_employe typee, users.id id,employes.mot_de_passe mdp,employes.etat_mot_de_passe etat_mdp FROM users INNER JOIN employes ON users.id = employes.id_user INNER JOIN agences ON agences.id = employes.id_agence WHERE employes.pseudo LIKE BINARY :ps",
            {
