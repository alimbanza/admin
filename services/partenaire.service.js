...
module.exports={
                ...
                if (err) { 
                    ...
                    await sequelize.transaction(async (t) => {
                        ...
                        let numPhone= part.dataValues.telephone,
                            ...
                            vient d'être enregistrée avec succès 
                            IM Gérant: ${code_partenaire+ '-' + code_us}
                            Merci.`
    ...
    find_gerant : async (Part)=>{
        ...
        if(Part.rep == true) {
            ...
            const [part, metada] = await sequelize.query("SELECT users.*,membres.id id_memb,etat_civils.libelle lb FROM users INNER JOIN etat_civils ON etat_civils.id=users.id_etat_civil INNER JOIN membres ON membres.id_user=users.id INNER JOIN type_membres ON type_membres.id=membres.id_type_membre WHERE users.numero_piece=:num_piece AND users.id_type_piece_ident=:id_type AND type_membres.id=6",
            {
    ...
    find_user : async (Part)=>{
        ...
        if(user == ''){
            const [employe, metada] = await sequelize.query("SELECT users.*,etat_civils.libelle lb,type_employes.libelle FROM users INNER JOIN employes ON employes.id_user=users.id INNER JOIN type_employes ON type_employes.id=employes.id_type_employe INNER JOIN etat_civils ON etat_civils.id=users.id_etat_civil WHERE users.numero_piece=:num_piece AND users.id_type_piece_ident=:id_type",
            {
                replacements: { num_piece :Part.num_piece, id_type:Part.type_piece} 
            })
